...
class InvityAPI {
    ...
    getAllApiServerUrls() {
        return [this.servers.production.api, this.servers.staging.api, this.servers.localhost.api];
    }
    ...
    private getInvityAuthenticationPageSrc(flow: 'login' | 'registration') {
        ...
        const url = new URL(
            isDesktop()
                ...
                : `${window.location.origin}/static/invity-authentication/${flow}.html`,
        );
        const returnToUrl = isDesktop()
            ? `http://localhost:21335/invity-${flow}-success`
            : `${window.location.origin}/static/invity-authentication/${flow}-success.html`;
